      ...
      <div class="flex flex-grow flex-col">
        ...
        <div class="mt-4 overflow-x-scroll">
          <div class="flex h-44 w-56 flex-row gap-5 sm:w-full md:h-48">
            <ConcertCard
              v-for="concert in listeConcert.filter((i) => i.jour === 'vendredi')"
              color="#CE6848"
              :key="concert.id"
        ...
        <div class="mt-4 overflow-x-scroll">
          <div class="flex h-44 w-56 flex-row gap-5 sm:w-full md:h-48">
            <ConcertCard
              v-for="concert in listeConcert.filter((i) => i.jour === 'samedi')"
              :key="concert.id"
              color="#245480"
              :artiste="concert.artiste"
              :horaire="concert.horaire"
        ...
        <div class="mt-4 overflow-x-scroll">
          <div class="flex h-44 w-56 flex-row gap-5 sm:w-full md:h-48">
            <ConcertCard
              v-for="concert in listeConcert.filter((i) => i.jour === 'dimanche')"
              :key="concert.id"
              color="#E2B87A"
              :horaire="concert.horaire"
              :session="concert.session"
