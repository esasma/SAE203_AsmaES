            ...
            <div class="flex flex-grow flex-col">
                ...
                <div class="mt-4 overflow-x-scroll">
                    <div class="flex h-32 w-[410px] flex-row gap-5 sm:w-full md:h-48">
                        <ConcertCard v-for="concert in listeConcert.filter(i => i.jour === 'vendredi')" color="#00ff00" :key="concert.id" :artiste="concert.artiste" :horaire="concert.horaire" :session="concert.session"/>
                    </div>
                </div>
                <h2 class="mt-9 font-barlow text-2xl font-bold uppercase">Samedi</h2>
                <div class="mt-4 overflow-x-scroll">
                    <div class="flex h-32 w-[410px] flex-row gap-5 sm:w-full md:h-48">
                        <ConcertCard v-for="concert in listeConcert.filter(i => i.jour === 'samedi')" :key="concert.id" color="#004f00" :artiste="concert.artiste" :horaire="concert.horaire" :session="concert.session"/>
                    </div>
                </div>
                <h2 class="mt-9 font-barlow text-2xl font-bold uppercase">Dimanche</h2>
                <div class="mt-4 overflow-x-scroll">
                    <div class="flex h-32 w-[410px] flex-row gap-5 sm:w-full md:h-48">
                        <ConcertCard v-for="concert in listeConcert.filter(i => i.jour === 'dimanche')" :key="concert.id" color="#00f580" :horaire="concert.horaire" :session="concert.session" :artiste="concert.artiste"/>
                    </div>
                </div>
            </div>
        </div>
    ...
    export default {
        ...
        methods: {
            async getConcert() {
                ...
                const q = query(dbConcert, orderBy('horaire'));
